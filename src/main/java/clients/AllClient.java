...
public class AllClient {
    public static void main(String[] args) {
        ...
        try {
            List<String> agentIds = DataLoader.getAllLocomotiveAgentIds();
            for (String agentId : agentIds) {
                AgentController agent = container.createNewAgent(
                        agentId, "agents.LocomotiveAgent", new Object[]{agentId});
                agent.start();
            }
            System.out.println("LocomotiveClient started " + agentIds.size() + " agents on " + host);

            agentIds = DataLoader.getAllWagonAgentIds();
            for (String agentId : agentIds) {
                AgentController agent = container.createNewAgent(
            ...
            agentIds = DataLoader.getAllCargoAgentIds();
            for (String agentId : agentIds) {
                AgentController agent = container.createNewAgent(
